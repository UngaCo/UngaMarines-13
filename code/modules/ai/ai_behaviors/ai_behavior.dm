...
/datum/ai_behavior/proc/ai_do_move()
	if(!mob_parent?.canmove || mob_parent.lying_angle && !mob_parent.can_crawl || mob_parent.do_actions) //todo: some do_actions allow movement, unsure if there is a way to trace this though
		return
